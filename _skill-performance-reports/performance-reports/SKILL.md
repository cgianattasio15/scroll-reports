...
**Unified Scoring Framework v3.0 (Monthly Account Score + Outlier Engine — same hierarchy):**
Both the monthly account score and the top post outlier engine use the same intent-weighted metric hierarchy. The core principle: the goal of organic social is to reach the highest-quality audience (buyers), not the widest audience. Metrics that require a deliberate decision from the viewer are weighted highest.
- **T1 (1.5x):** Saves, Retention %, Link Taps, PCR, CTR — highest-intent buyer signals. These require a deliberate viewer decision. Saves = pre-purchase bookmarking. Link Taps & CTR = BOFU action. Retention % = content resonance. PCR = profile-to-follower conversion.
...
- **T3 (0.75x):** Comments — supporting signal, highly variable by niche.
...
- **Master Framework reference:** `Scroll Media — Master Metrics & Scoring Framework v3.0.md` in project files is the canonical source of truth for all tier assignments, target ranges, and scoring logic.
...
- `references/template-token-reference.md` — **Token reference sheet (v1.0).** Defines all 64 `{{TOKEN}}` placeholders in the canonical template. Use as the fill-in checklist when building a new report. Includes source for each token, status value options, MoM calculation formulas, and pre-deployment checklist.
- `templates/canonical-template.html` — **Tokenized HTML template (v1.0).** A fully tokenized version of the canonical report structure with all client-specific content replaced by `{{TOKEN}}` placeholders. Use this as the starting point for every new report. Copy to `/scroll-reports-repo/[client-slug]/[monthyear]/index.html` and fill in all tokens using the token reference sheet.

## Data Verification Gate

**MANDATORY: Run before deploying any report.** The verification script at `scripts/verify_report_data.py` cross-checks every metric value in the report HTML against the Master Performance Data Sheet. It catches stale cache data, wrong MoM deltas, and mismatched values before they reach clients.

```bash
python3 /home/ubuntu/skills/performance-reports/scripts/verify_report_data.py
```

The script checks all 7 active clients by default. A clean run shows `PASS` for all clients. Any `FAIL` must be resolved before deploying. This script is the primary safeguard against the data accuracy issues that caused SBB (694% MoM error), DEFINE, and MEAS errors in March 2026.
...
**Deployment gate (non-negotiable order):**
1. Build report from `templates/canonical-template.html`
2. Run `verify_report_data.py` — must pass before proceeding
3. Run QA script — must pass before deploying
4. Push to GitHub — Netlify auto-deploys
5. Verify live at `reports.scrollmedia.co/[client-slug]/[monthyear]/`
